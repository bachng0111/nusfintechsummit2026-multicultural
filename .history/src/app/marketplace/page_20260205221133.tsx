...
import { fetchMarketplaceTokens, removeMarketplaceToken } from '@/lib/tokens-api';
...
export default function MarketplacePage() {
  ...
  // Load minted tokens from external API
  useEffect(() => {
      ...
      try {
        ...
        if (stored) {
          try {
            ...
            console.error('Failed to parse stored tokens:', parseErr);
          }
        }
      }
    };
  ...
  const handleCreateEscrowAndPay = async (request: PurchaseRequest) => {
    ...
    try {
      ...
      // Remove the purchased token from marketplace via API and update state
      // Note: The token metadata is preserved in the archive for buyer's account page
      try {
        await removeMarketplaceToken(request.tokenIssuanceId);
        // Refresh tokens from API
        const updatedTokens = await fetchMarketplaceTokens();
        setTokens(updatedTokens);
      } catch (err) {
        console.error('Failed to remove token from marketplace:', err);
        // Fallback: update local state anyway
        setTokens(prev => prev.filter(t => t.issuanceId !== request.tokenIssuanceId));
      }
